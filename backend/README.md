...
| activateClass(classId, userId)                  | Only faculty can access, we update the lateTime, status, activatedBy, endTime | MVP               |
| endClass(classId, userId)                       | Only faculty can access, we update the status to finished.   |                   |
